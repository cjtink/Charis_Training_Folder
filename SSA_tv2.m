function [x,xstor,tstor] = SSA_tv2(tstop, x0, t0, S, W1, W0, Wt, i)
...
%         W = W1*x + W0;
%     elseif t == t0
        W = get_prop(x,t,W0,W1,Wt,tv_param_ind);
        kfast = 1.1*sum(W);
        W = [W;kfast];
%    end
...
Wnew = W1*x + W0;
% only want to change the second entry of this vector. 
Wnew(i) = max([0,Wt(t)*x(2)]); %replaces necessary W value with the 
W = Wnew;
